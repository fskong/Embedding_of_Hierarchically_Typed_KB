...
public:
	...
	double updateTypeGradient(uvec &posIndices, boost::minstd_rand rng)
	{
		...
		for (int i =0; i < Xp.n_cols; i++)
		{
			...
			De.rows(en.first) = (De.rows(en.first) +2 * lambda * delta * temp) ;
			...
			De.rows(en.second) = (De.rows(en.second) + 2 * lambda* delta *  temp);
			...
			if(neg_loss<type_margin)
			{
				...
				De.rows(neg_edge) = (De.rows(neg_edge) -2 * lambda* delta *  temp);
	...
	double updateRelationTypeGradient(int rel, uvec &posIndices, boost::minstd_rand rng)
	{
		...
		vec ga = 2 * Xp_r.t() * posTmp;
		ga -= 2 * Xn_r.t() * negTmp;
		At[rel] =normalise(At[rel] - reldelta*ga);
